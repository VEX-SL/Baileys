...
const validateConnection = async () => {
    const supportedVersions = WAProto_1.proto.HandshakeMessage.encode({ 
        clientHello: { ephemeral: ephemeralKeyPair.public } 
    }).finish();
    
    const result = await awaitNextMessage(supportedVersions);
    const handshake = WAProto_1.proto.HandshakeMessage.decode(result);
    
    const { serverHello } = handshake;
    if (!serverHello) throw new boom_1.Boom('Invalid handshake');
 ...
 const uploadPreKeys = async (count = Defaults_1.INITIAL_PREKEY_COUNT) => {
    await keys.transaction(async () => {
        ...
        // الصيغة الجديدة لرفع المفاتيح
        node.attrs.xmlns = 'encrypt';
        node.attrs.to = WABinary_1.S_WHATSAPP_NET;
